...
class optimizador:
    def __init__(self, f_name, epsilon, iter=100, function_type='objetive'):
        self.funcion = 0
        self.tipo = function_type
        self.epsilon = epsilon
        self.iteraciones = iter
        self.name = f_name
        self.funcion = self.get_function(f_name)

    def get_function(self, fname):
        if self.tipo == 'objetive':
            obj_func_instance = objetive_function(fname)
        elif self.tipo == 'restriction':
            obj_func_instance = restriction_fuctions(fname)
        elif self.tipo=='gradiente':
           funcion=self.name
           return funcion
        else:
            raise ValueError(f"Invalid function type: {self.tipo}")
        
        func = obj_func_instance.get_function()
        if func is None:
            raise ValueError(f"Function '{fname}' is not defined in the class '{self.tipo}'.")
        return func
class optimizador_univariable(optimizador):
    def __init__(self, x_inicial, xlimite, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(f_name, epsilon, iter, function_type)
...
class optimizador_multivariable(optimizador):
    def __init__(self, variables, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(f_name, epsilon, iter, function_type)
        self.variables = variables

class by_regions_elimination(optimizador_univariable):
    def __init__(self, x_inicial, xlimite, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(x_inicial, xlimite, f_name, epsilon, iter, function_type)
...
class direct_methods(optimizador_multivariable):
    def __init__(self, variables, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(variables, f_name, epsilon, iter, function_type)
...
class interval(by_regions_elimination):
    def __init__(self, x_inicial, x_limite, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(x_inicial, x_limite, f_name, epsilon, iter, function_type)
...
class neldermead(direct_methods):
    def __init__(self, variables, gamma, beta, f_name, epsilon, iter=100, function_type='objetive'):
        super().__init__(variables, f_name, epsilon, iter, function_type)
...
class hooke_jeeves(direct_methods):
    def __init__(self, variables, f_name, epsilon, iter=100, function_type='objetive', alpha=2, delta=None):
        super().__init__(variables, f_name, epsilon, iter, function_type)
        self.alpha = alpha
        self.delta = delta
...
class cauchy(gradient_methods):
    def __init__(self, variables, f_name, epsilon, epsilon2,iter=100, function_type='objetive',opt: optimizador_univariable = goldensearch):
        super().__init__(variables, f_name, epsilon, iter, function_type)
    ...
    def optimizaralpha(self,test):
        opt=self.opt(0,max(self.variables) ,test,self.epsilon,100,'gradiente')
        alfa=opt.optimize()
        return alfa
    def optimize(self):
        terminar = False
...
class fletcher_reeves(gradient_methods):
    def __init__(self, variables, f_name, epsilon,epsilon2,epsilon3, iter=100, function_type='objetive',opt:optimizador_univariable = goldensearch):
        super().__init__(variables, f_name, epsilon, iter, function_type)
        self.epsilon2 = epsilon2
...
if __name__=="__main__":
  # Define los parámetros para interval
    x_inicial = 0.1  # Ejemplo de punto inicial
    x_limite = 10  # Ejemplo de límite
    f_name = 'funcion1'  # Nombre de la función a optimizar
    epsilon = 0.01  # Precisión

    # Crea una instancia de interval
    opt_interval = interval(x_inicial, x_limite, f_name, epsilon)#Funciona
    opt_fibonacci=fibonacci(x_inicial, x_limite, f_name, epsilon)#Funciona
    opt_golden=goldensearch(x_inicial, x_limite, f_name, epsilon)#Funcion
    opt_newton=newton_raphson(x_inicial, x_limite, f_name, epsilon)#Funciona
    opt_biseccion=biseccion(x_inicial, x_limite, f_name, epsilon)#Funciona 
    opt_secante=secante(x_inicial, x_limite, f_name, epsilon)#Funciona
    #Multivariable directos
    variables_iniciales = [0, 0]
    epsilon = 1e-6
    iteraciones = 1000
    funcion_objetivo = "himmelblau"
    opt_rw = random_walk(variables_iniciales, funcion_objetivo, epsilon, iteraciones)#funciona
    resultado = opt_rw.optimize()
    #Hookejeeves 
    #Nelder mead
    ...
    e=0.5
    f="rosenbrock"
    nelder=neldermead(initialpoint, gamma, b, f, e)#funciona
    inicio = [1, 1]
    n=[2,3]
    e = 0.001
    cauchy_instance = cauchy(inicio, "himmelblau", e, e, iter=100, opt=goldensearch)
    newton_instance=newton(n, "himmelblau", e, e, iter=100, opt=goldensearch)
    # Parámetros iniciales
    inicio = [1, 1]
    e = 0.001
    iter = 100

    # Crear una instancia de la clase fletcher_reeves
    gradiente_instance = fletcher_reeves(inicio, "himmelblau", e, e, e, iter=iter, opt=goldensearch)

    # Ejecutar la optimización
    resultado = gradiente_instance.optimize()
